...
public class J2TeamValidator {
    ...
    public static final Validator JSON_FORMAT_VALIDATOR = (subject, input, context) -> {
        ...
        try {
            ...
            Assert.isTrue(allowUrl.contains(j2TeamCookies.getUrl()), "Необходимо куки от login VK. Получено: \"" + j2TeamCookies.getUrl() + "\", разрешено: " + allowUrl.stream().collect(Collectors.joining("\", \"", "[\"", "\"]")) + ".");
